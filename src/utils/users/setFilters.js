...
module.exports = userFilters => {
    let filters = {}
    const { fname, lname, email, gender, status, systemRole, startDate, endDate } = userFilters

    if (startDate && !validateSimpleDate(startDate)) throw new Error(`Invalid format ${startDate}`)

    if (endDate && !validateSimpleDate(endDate)) throw new Error(`Invalid format ${endDate}`)
     ...
     */
    if (startDate && endDate) filters['enrollmentDate'] = { [Op.between]: [startDate, endDate] }
