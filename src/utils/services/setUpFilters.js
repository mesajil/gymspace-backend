...
module.exports = filters => {
    ...
    if (endTime && !isValidHourMinuteFormat(endTime)) throw new Error('Invalid endTime format hour:minute (00:00).')

    if (startDuration && isValidPositiveInteger(startDuration))
        throw new Error(`startDuration must be a positive number.`)

    if (endDuration && isValidPositiveInteger(endDuration)) throw new Error(`endDuration must be a positive number.`)

    if (minCapacity && isValidPositiveInteger(minCapacity)) throw new Error(`minCapacity must be a positive number.`)

    if (maxCapacity && isValidPositiveInteger(maxCapacity)) throw new Error(`maxCapacity must be a positive number.`)
    ...
    if (startTime && endTime) servicesOptions['startTime'] = { [Op.between]: [startTime, endTime] }
    else if (startTime) servicesOptions['startTime'] = { [Op.gte]: startTime }
    else if (endTime) servicesOptions['startTime'] = { [Op.lte]: endTime }

    if (startDuration && endDuration) servicesOptions['duration'] = { [Op.between]: [startDuration, endDuration] }
    ...
    if (minCapacity && maxCapacity) servicesOptions['capacity'] = { [Op.between]: [minCapacity, maxCapacity] }
