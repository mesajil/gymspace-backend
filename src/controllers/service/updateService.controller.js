...
const updateService = async (req, res) => {
    try {
        const data = req.body
        const { id } = req.params
        const { name, description, category, price, startTime, duration, image, status, capacity, coachID, areaID } =
            req.body

        // Data Type Validation

        if (price && isNaN(price)) return res.status(404).json({ error: 'price is not a number' })
        ...
        if (image && !isValidImageUrl(image)) return res.status(404).json({ error: 'Invalid image url.' })

        if (id && !isValidUUID(id)) return res.status(404).json({ error: 'Id must have UUID format.' })

        // Uniqueness Validation

        const sameNameService =
            name &&
        ...
        if (sameNameService) return res.status(404).json({ error: 'Name already registered' })
