...
const getCart = async (req, res) => {
    try {
        ...
        return res.status(200).json({ products: productsCartResponse, services: servicesCartResponse })
    } catch (error) {
        console.error(error)
        return res.status(500).json({ message: 'Error getting the cart', error: error.message })
    }
}
