...
const updateUser = async (req, res) => {
    try {
        ...
        if (!user) return res.status(404).json({ error: 'Id user does not exist' })

        const { email, birth, enrollmentDate, phone, contactPhone, favoriteProducts, favoriteServices } = data

        if ('email' in data) {
            if (!isValidEmail(email)) return res.status(404).json({ error: 'Invalid email' })
            ...
            if (sameEmailUser) return res.status(404).json({ error: 'Email already registered' })
        }

        if ('birth' in data && !validateSimpleDate(birth)) return res.status(404).json({ error: 'Invalid date format' })

        if ('enrollmentDate' in data && !validateSimpleDate(enrollmentDate))
            return res.status(404).json({ error: 'Invalid date format' })

        if ('phone' in data) {
            if (!isValidPhoneNumber(phone)) return res.status(404).json({ error: 'Invalid phone number' })
            ...
            if (samePhoneUser) return res.status(404).json({ error: 'Phone already registered' })
        }
